...
for ((i = 1; i <= 15; i++)); do
	...
	pid=$!
	(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null &
	watcher=$!
	wait $pid 2>/dev/null
	if [[ -s stdout ]]; then
...
for i in {1..5}; do
	...
	(leaks -atExit -- $ROOT/push_swap "$(cat $TRACE/size3/test_case_$i.txt)" 1>a 2>b) &
	pid=$!
	(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null &
	watcher=$!
	wait $pid 2>/dev/null
...
for i in {1..119}; do
	...
	rm -rf stdout stderr result error_result ok aa
	(leaks -atExit -- $ROOT/push_swap "$(cat $TRACE/size5/test_case_$i.txt)" 1>a 2>b) &
	pid=$!
	(sleep $TIME_LIMIT && kill -HUP $pid) 2>/dev/null &
	watcher=$!
	wait $pid 2>/dev/null

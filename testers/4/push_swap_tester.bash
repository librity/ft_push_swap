...
function test() {
	...
	if [[ $1 -eq 3 ]]; then
		...
		if [[ $INS -le 700 ]]; then
			SCORE="5"
		elif [[ $INS -le 900 ]]; then
			SCORE="4"
		elif [[ $INS -le 1100 ]]; then
			SCORE="3"
		elif [[ $INS -le 1300 ]]; then
			SCORE="2"
		elif [[ $INS -le 1500 ]]; then
			SCORE="1"
		else
			SCORE="0"
			INS_RESULT="FAIL"
		...
		if [[ $INS -le 5500 ]]; then
			SCORE="5"
		elif [[ $INS -le 7000 ]]; then
			SCORE="4"
		elif [[ $INS -le 8500 ]]; then
			SCORE="3"
		elif [[ $INS -le 10000 ]]; then
			SCORE="2"
		elif [[ $INS -le 11500 ]]; then
			SCORE="1"
		else
			SCORE="0"
			INS_RESULT="FAIL"
...
if [[ -z $(make -C $PUSHSWAP_DIR 2>&1 >/dev/null) ]]; then
	echo $GREEN"  Building Successs!"$WHITE
else
	echo $RED"  Building Failed!"$WHITE
	exit 1
